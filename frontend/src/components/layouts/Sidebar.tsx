...
export default function Sidebar() {
    ...
    return (
        <aside className="fixed left-0 top-0 h-screen w-64 glass border-r border-slate-700/50 flex flex-col z-50">
            ...
            {/* Navigation — Fixed: no nested <a> inside <Link> */}
                ...
                {bottomNavItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.path}
                            href={item.path}
                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 transition-all duration-200"
                        >
